...
logging.getLogger("tensorflow").setLevel(
    ...
    def predict_activities_3groups_arm():
    ...
    def define_model():
        model = Sequential([
            keras.layers.InputLayer(shape=(Activity, Dimensions)),
    ...
    results_walking = pd.DataFrame.from_dict(results_walking, orient='index', columns=['selected_batch', 'selection', 'n_epochs', 'train_acc',
                                                                                    ...
                                                                                    'weighted_precision', 'weighted_f1_score', 'confusion_mat',
                                                                                    'normalized_confusion_mat'])
    results_walking.to_excel('Results/results_walking_arm_l3o.xlsx')
    ...
    results_Nwalking = pd.DataFrame.from_dict(results_Nwalking, orient='index', columns=['selected_batch', 'selection', 'n_epochs', 'train_acc',
                                                                                        ...
                                                                                        'weighted_precision', 'weighted_f1_score', 'confusion_mat',
                                                                                        'normalized_confusion_mat'])
    results_Nwalking.to_excel('Results/results_Nwalking_arm_l3o.xlsx')
    ...
    for name, df in results.items():
        ...
        normalized_confusion_mat = df['normalized_confusion_mat'].copy()
        for idx, matrix in normalized_confusion_mat.items():
            for j in matrix:
                if ((np.isnan(j).any()) or (len(j) < 3)):
        ...
        sns.heatmap(normalized_confusion_mat, annot=True,  cmap="Blues", cbar=False, square=True,
                    xticklabels=["Stationary",
                                "Locomotion", "Wheeling & Handbiking"],
                    yticklabels=["Stationary", "Locomotion", "Wheeling & Handbiking"], ax=ax)
        ...
        fig.tight_layout()
        fig.savefig(
            f'Figures/Confusion_matrix_3groups_arm_{name}.png', dpi=400)
