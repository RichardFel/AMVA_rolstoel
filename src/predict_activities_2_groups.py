...
from utils import z_score, detect_outliers, remove_outliers
...
def predict_activities_2groups():
    # Settings
    ...
    results_all.to_excel('Results/Results_arm_ankle_l2o.xlsx')

    results_walking = pd.DataFrame.from_dict(results_walking, orient='index', columns=['selected_batch', 'selection', 'n_epochs', 'train_acc',
                                                                                       'val_acc', 'overall_accuracy', 'weighted_recall',
                                                                                       'weighted_precision', 'weighted_f1_score', 'confusion_mat',
                                                                                       'normalized_confusion_mat'])
    results_walking.to_excel('Results/results_walking_arm_ankle_l2o.xlsx')
    ...
    results_Nwalking = pd.DataFrame.from_dict(results_Nwalking, orient='index', columns=['selected_batch', 'selection', 'n_epochs', 'train_acc',
                                                                                         ...
                                                                                         'weighted_precision', 'weighted_f1_score', 'confusion_mat',
                                                                                         'normalized_confusion_mat'])
    results_Nwalking.to_excel('Results/results_Nwalking_arm_ankle_l2o.xlsx')
    ...
    for name, df in results.items():
        ...
        fig.tight_layout()
        fig.savefig(
            f'Figures/Confusion_matrix_all_children_arm_ankle_2groups_{name}.png', dpi=400)
