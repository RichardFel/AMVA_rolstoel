...
old = charact.loc[(charact['Leeftijd'] > 12), 'Proefpersoonnummer'].unique()
old = unique_part[np.isin(unique_part, old)]
walking = charact.loc[(charact['Onderzoeksgroep'] == 3), 'Proefpersoonnummer'].unique()
walking = unique_part[np.isin(unique_part, walking)]
nWalking = charact.loc[(charact['Onderzoeksgroep'] == 4), 'Proefpersoonnummer'].unique()
...
results_all.to_excel('Results/Results_arm_ankle_l3o.xlsx')

results_walking = pd.DataFrame.from_dict(results_walking, orient='index', columns=['selected_batch', 'selection', 'n_epochs', 'train_acc',
                                                                           'val_acc', 'overall_accuracy', 'weighted_recall',
                                                                           'weighted_precision', 'weighted_f1_score', 'confusion_mat',
                                                                           'normalized_confusion_mat'])
results_walking.to_excel('Results/results_walking_arm_ankle_l3o.xlsx')
...
for name, df in results.items():
    ...
    ax.set_title(f"Confusion matrix arm and ankle sensor")

    fig.tight_layout()
    fig.savefig(
        f'Figures/Confusion_matrix_3groups_arm_ankle_{name}.png', dpi=400)
